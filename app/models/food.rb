class Food < ApplicationRecord
  ...
  # 入力日以前前且つ使い切り期限より先のお知らせ日は入力不可
  def noticedate_before_inputdate_and_expirationdate_is_not_possible
    if notice.present? && notice <= Date.today && notice > expiration
      errors.add(:notice, "本日以前、且つ使い切り期限より先の日付は入力できません")
    end
  end
